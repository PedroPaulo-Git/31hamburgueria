

<template>
  <section class="bg-gray-50">
    <div class="mx-auto max-w-screen-xl px-4 py-32 lg:flex lg:h-screen lg:items-center">
      <div class="mx-auto max-w-xl text-center">

        <div class="px-0 text-center ">
        <span id="h1-about" class="text-3xl font-extrabold sm:text-5xl">
          Não vendemos apenas hambúrgueres, 
          <h1 id="h1-about" class="text-5xl font-extrabold sm:text-5xl text-yellow31 sm:block">
            entregamos experiências únicas.</h1>
          </span>
        <img
            src="../../assets/tracobanner.png"
            class="w-32 mx-auto -mt-2 mb-10"
            alt=""
          />
      </div>
       
  



        <div class="About mt-20 ">


          <h1 id="h1-about" class="text-5xl my-6 font-extrabold sm:text-5xl text-black sm:block">
            Sobre nós
          </h1>
          <img
            src="../../assets/traco2.png"
            class="w-52 h-40 mx-auto -mt-20 -mb-14"
            alt=""
          />
          
          <img src="../../assets/traco1.png" class="w-20 -rotate-12 " alt="">
          <img class="rounded-full h-28 w-full object-cover" src="../../assets/bannerABout.jpg" alt="">
          <p class="text-gray-600 text-sm mt-6">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Quod laboriosam architecto minus rerum iure nisi, magnam, asperiores corrupti vero ipsum deserunt quasi excepturi dicta quisquam? Praesentium iure saepe assumenda rem.
          </p>
        <div class="mt-8 flex flex-wrap justify-center gap-4">
          <a
            class="block w-40 rounded-full bg-yellow31 px-12 py-3 text-sm font-medium text-black shadow hover:bg-red-700 focus:outline-none focus:ring active:bg-red-500 sm:w-auto"
            href="#"
          >
            Saber Mais
          </a>
        </div>
        </div>
     
      </div>
    </div>
  </section>
</template>
<script>

</script>
<style>
    
</style>